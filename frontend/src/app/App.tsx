...
export function App() {
      ...
      setGame((value) => {
        ...
        return {
          ...
          log: nextEnemy.kind === 'boss'
            ? `Врата города открываются. На площадь выходит босс — ${nextEnemy.name}.`
            : `${nextEnemy.name} появляется на следующей улице. Продолжай атаку.`,
        };
      });
      setTransitioning(false);
      swipeCombo.current = 0;
    }, enemy.kind === 'boss' ? 1450 : 700);
  ...
  const performAction = (action: ActionType) => {
    ...
    setGame((value) => ({
      ...
      log: 'Защитная стойка: следующий удар ослаблен.',
    }));
  ...
  const pointerMove = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (now - lastSwipeHit.current > 125) {
  ...
  const selectFloor = (floorId: number) => {
    ...
    setGame((value) => ({
      ...
      enemy: createEnemy(floorId, value.player.floorWins[floorId] ?? 0),
      battleWon: false,
      gameOver: false,
      log: `Ты вошёл в локацию «${FLOORS[floorId - 1].name}». Проведи мечом по экрану.`,
    }));
    setView('explore');
  };
  ...
  const repair = () => {
    ...
    setGame((value) => ({
      ...value,
      player: { ...value.player, col: value.player.col - repairPrice, weaponDurability: value.player.maxWeaponDurability, armorDurability: value.player.maxArmorDurability },
      log: 'Оружие и доспехи полностью починены.',
    }));
  };
        ...
        {view === 'explore' && <section className="battle-layout cinematic-battle-layout">
          <section className={`battle-stage game-panel swipe-battle-stage ${hitFlash ? 'enemy-hit' : ''}`}>
            ...
            <div className={`battle-scene swipe-city-scene floor-${player.selectedFloor}`} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp}>
              ...
              <div className={`rear-enemy ${enemy.kind} ${battleWon ? 'defeated' : ''}`}><div className="enemy-shadow"/><div className="enemy-body"><span>{enemy.kind === 'boss' ? '👹' : enemy.kind === 'elite' ? '🐺' : '🐗'}</span></div></div>
              ...
              <div className="swipe-hint">ПРОВЕДИ МЕЧОМ ПО ЭКРАНУ</div>
